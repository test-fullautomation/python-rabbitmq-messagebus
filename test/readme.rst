...
The test files used for testing, are placed in folder
...
be chosen for execution by providing the test id (``TESTID``) in command line in the following way:
...
The execution of all test cases runs in a loop. The **<rabbitmqMessagebus_EventBusClient>** class object is created (and deleted) outside this loop (default!).

Outcome: *All tests work with the same class object.*

With the (optional) command line switch ``--recreateinstance`` the **<rabbitmqMessagebus_EventBusClient>** class object is created (and deleted) inside the loop.
...
* The values returned from **<rabbitmqMessagebus_EventBusClient>** (like defined with ``EXPECTEDRETURN`` in ``TestConfig.py``)
...
It is expected, that either values are returned from **<rabbitmqMessagebus_EventBusClient>** or an exception is thrown.
...
.. code::
   ...
   pytest/pytestfiles/test_03_FEATURE_A_GOODCASE.py

Class name inside the ``pytest`` file containing the call of this test:

.. code::

   class Test_FEATURE_A_GOODCASE:
