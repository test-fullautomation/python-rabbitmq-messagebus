...
def AnalyzeReturnedValues(EXPECTEDRETURN=None, actualReturned=None):
   ...
   if ( (EXPECTEDRETURN is None) and (actualReturned is None) ):
      ...
      sResult  = "EventBusClient returned None, but values are expected"
      listErrors.append(sResult)
   elif ( (EXPECTEDRETURN is None) and (actualReturned is not None) ):
      sResult  = "EventBusClient returned values, but values are not expected"
      listErrors.append(sResult)
   else:
      ...
      if nNrOfLinesReturned != nNrOfLinesExpected:
         bSuccess = False
         sResult  = f"EventBusClient value counter mismatch! Expected: {nNrOfLinesExpected} values, but returned: {nNrOfLinesReturned} values"
         listErrors.append(sResult)
   ...
   # eof else - elif - if ( (EXPECTEDRETURN is None) and (actualReturned is None) ):

   if len(listErrors) == 0:
      bSuccess = True
      if EXPECTEDRETURN is None:
         sResult  = "No values returned from EventBusClient (like expected)."
      else:
         sResult  = "EventBusClient returned expected values."
   else:
      bSuccess = False
      sResult  = "EventBusClient did not return expected values."
...
def AnalyzeExceptions(EXPECTEDEXCEPTION=None, sException=None):
   ...
   if ( (EXPECTEDEXCEPTION is None) and (sException is None) ):
      ...
      sResult  = "EventBusClient threw no exception, but an exception is expected"
      listErrors.append(sResult)
   elif ( (EXPECTEDEXCEPTION is None) and (sException is not None) ):
      sResult  = "EventBusClient threw an exception, but an exception is not expected"
      listErrors.append(sResult)
   else:
      ...
      if EXPECTEDEXCEPTION not in sException:
         ...
         bSuccess = False
         sResult  = f"EventBusClient exception mismatch! Expected exception:\n'{EXPECTEDEXCEPTION}',\nbut thrown:\n'{sException}'"
         listErrors.append(sResult)
   ...
   if len(listErrors) == 0:
      ...
      if EXPECTEDEXCEPTION is None:
         sResult  = "No exception thrown from EventBusClient (like expected)."
      else:
         sResult  = "EventBusClient threw expected exception."
   else:
      bSuccess = False
      sResult  = "EventBusClient did not throw expected exception."
...
for dictUsecase in listofdictUsecases:
   ...
   # Checking the EventBusClient depends on the following two return values: 'actualReturned' and 'sException'.
