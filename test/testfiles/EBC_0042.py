...
# EBC_0042.py
#
# Test case for EBC_0042: Headers exchange with match_all=True (AND logic)
#
# --------------------------------------------------------------------------------------------------------------
...
async def test(config_folder_path):
    """
    Test case EBC_0042: Headers exchange with match_all=True (AND logic).

    This test verifies that when using HeadersExchangeHandler with match_all=True,
    messages are only delivered to subscribers when ALL specified headers match.
    ...
    received_messages = []
    test_message_content = "Hello, Headers Exchange!"
    ...
    try:
        ...
        # Subscribe with binding_headers using match_all=True (AND logic)
        # Message must have BOTH format=pdf AND department=engineering to be received
        ...
        # Send message with matching headers (should be received)
        ...
        try:
            ...
            result = f"Message received with AND logic: {received_messages[0]}"
        ...
        print(e)
        result = f"Error during headers exchange test: {str(e)}"
        return result, oEventBusClient
