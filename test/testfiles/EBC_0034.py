...
# EBC_0034.py
#
# Test case for EBC_0034: Test ControlMessage with invalid instance_id data type
#
# --------------------------------------------------------------------------------------------------------------
...
async def test(config_folder_path):
    """
    Test case EBC_0034: Test ControlMessage with invalid instance_id data type.

    This test verifies that ControlMessage should validate input and raise TypeError
    when instance_id is not a string but another data type like int, list, dict, etc.

    The implementation should validate that:
    1. instance_id parameter must be a string or None
    ...
    try:
        # This should raise TypeError since instance_id is an int instead of string
        message = ControlMessage(
            kind="test_kind",
            roles=["test_role"],
            instance_id=12345  # Should be string, not int
        )
        ...
        return f"BUG: ControlMessage accepted invalid instance_id type 'int' when string was expected", None

    except TypeError as e:
        # This is what SHOULD happen when the implementation is correct
